...
const exemplaState: PlayerState = {
  course: {
    id: 1,
    modules: [
      {
...
describe('player slice', () => {
  it('should be able to play', () => {
    const state = reducer(exemplaState, play([1, 2]))
  ...
  it('should be able to play next video automatically', () => {
    const state = reducer(exemplaState, next())

    expect(state.currentModuleIndex).toEqual(0)
    expect(state.currentLessonIndex).toEqual(1)
  })

  it('should be able to play to jump to the next module automatically', () => {
    const state = reducer({
      ...exemplaState,
